...
let status=0,tokenBuyBackAdd,tokenBuyBackWalletAddress;
                    ...
                    if (symbol.chat.id === chatId) {
                        ...
                        try {
                            ...
                            const tokenSupplyFunction = async (supply) => {
                                ...
                                try {
                                    bot.sendMessage(chatId, 'Enter Buy Back Token Address');
                                    const tokenBuyBackAddress = async (buyBackAddress) => {
                                        if (buyBackAddress.chat.id === chatId) {
                                            tokenBuyBackAdd = buyBackAddress.text;
                                            bot.removeListener('text', tokenBuyBackAddress);
                                            try {
                                                bot.sendMessage(chatId, 'Enter Buy Back Wallet Address');
                                                const tokenBuyBackWallet = async (BuyBackWallet) => {
                                                    if (BuyBackWallet.chat.id === chatId) {
                                                        tokenBuyBackWalletAddress = BuyBackWallet.text;
                                                        bot.removeListener('text', tokenBuyBackWallet);
                                                        try {
                                                            const keyboard = {
                                                                inline_keyboard: [
                                                                    [{
                                                ...
                                                bot.on('text', tokenBuyBackWallet)
                                            } catch (error) {
                                                console.log("Error in charity Address", error);
                                            }
                                        }
                                    }
                                    bot.on('text', tokenBuyBackAddress)
                                } catch (error) {
                                    console.log("Error in Market Address", error)
                                }
...
bot.onText(/\/deploy/, async (msg) => {
    ...
    try {
        let contractaddress;
        if(status==1)contractaddress = await deployeBuyBackToken(tokenName, tokenSymbol, tokenSupply, tokenBuyBackAdd, tokenBuyBackWalletAddress, walletAdd, walletPrivateKey);
        else contractaddress = await deployeStandardToken(tokenName, tokenSymbol, tokenSupply, tokenCharityAddress, tokenMarketAddress, sellStructure, walletAdd, walletPrivateKey);
